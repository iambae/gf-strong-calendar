...
class SessionsMissed extends Component {
	state = {
	    // printButton: null,
	    table: null,
	...
	getTable = rawData => {
	    ...
	    // create table
	    return (
	        <Table striped bordered hover variant="light" id="session-missed-report">	            
	            <thead>
	                <tr>
	                    <th>Category</th>
	                    <th>PT</th>
	                ...
	                {result.categories.map((row, index) => {
	                    return (
	                        <tr key={index}>
	                            ...
	                            <td>{Math.round(row.PT.median_missed)}</td>
	                            <td>
	                                {Math.round(row["PT RA"].median_missed)}
	                            </td>
	                            <td>{Math.round(row.OT.median_missed)}</td>
	                            <td>
	                                {Math.round(row["OT RA"].median_missed)}
	                            </td>
	                            <td>{Math.round(row.SLP.median_missed)}</td>
	                            <td>{Math.round(row.SLPA.median_missed)}</td>
	                        </tr>
	                    );
	                })}
	            </tbody>
	            ...
	            <Row className="inner-row-btn">
	                <div className="inner-row-col">
	                    <ReactHTMLTableToExcel
	                        ...
	                        table="session-missed-report"
	                        filename="Sessions Missed Summary"
	                        sheet="report"
